...
function get_quiz_data() {
    ...
    xhr.open('GET', "Don't look/quiz1.json");
 ...
 */
 function register_choice_event() {
    for (var i = 0; i < document.querySelectorAll('.js-quiz-choice').length; i++) {
        ...
        }else if(currentQuizNo + 1 < quizData['quiz'].length) {
                // 次の問題へ遷移するときのイベント設定
                register_nextquiz_event();
            // 全て回答済の場合
            } else {
...
function generate_quiz_content() {
    ...
    ins +='<di class="choice">'
    if (currentQuizNo == 3){
    ins +='<audio controls src="src/Music/Solaceon Town(day).wav">Your browser does not support the<code>audio</code> element.</audio>'
    }
 ...
 */
 function generate_result_content() {
    ...
    ins += '<div class="p-quiz-next">';
        ins += '<a href="index.html"><button class="c-btn" o>トップに戻る</button></a>';
    ins += '</div>';
